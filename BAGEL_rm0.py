...
for loop in range(NUM_BOOTSTRAPS):
    ...
    ess_train_fc_flat_list = [obs for sublist in ess_train_fc_list_of_lists for obs in sublist if obs != 0] # remove 0
    #
    ...
    non_train_fc_flat_list = [obs for sublist in non_train_fc_list_of_lists for obs in sublist if obs != 0] # remove 0
    #
    ...
    for g in genes_array[gene_test_idx]:
        foldchanges = array([x for x in fc[g] if x != 0]) # remove 0
        if len(foldchanges) > 0:                          # length large than 0
...
fout.write('GENE\tBF\tSTD\tNumObs\tP\n')
for g in sorted( bf.keys() ):
    ...
    fout.write('{0:s}\t{1:4.3f}\t{2:4.3f}\t{3:d}\t{4:4.3f}\n'.format( g, bf_mean, bf_std, num_obs,pval ) )
fout.close()
